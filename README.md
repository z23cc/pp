...
`pp` turns one strict OpenAPI 3.0 YAML/JSON spec into a buildable Rust CLI/MCP workspace. Generated commands and MCP tools share native direct HTTP operation metadata and runtime code; no generated API-client crate or external codegen backend is required.

The Cargo package is `pp-cli`; the installed binary is `pp`. The project is preparing a `0.1.0` crates.io release; see `docs/release-status.md` for the current publication checklist.
...
MCP tool calls and human CLI commands use the same audited native direct HTTP adapter: generated operation metadata supplies the method, path template, path/query argument bindings, and JSON body bindings. Each generated workspace records the adapter in `pp-transform-plan.json` as `runtime.mcp_invocation.direct_http`.
...
prune, or relax typed OpenAPI shapes to satisfy code generation. Specs are parsed,
optionally sliced, inspected, modeled, and generated as-is; unsupported native
runtime shapes fail at model construction.
