...
- [x] Follow-up packet — auth selection policy: `inspect` and `generate` accept `--auth-policy legacy|fail-ambiguous` and `--auth-scheme <NAME>`; defaults now fail on ambiguous selectable component schemes, with legacy component-order selection only as explicit opt-in.
